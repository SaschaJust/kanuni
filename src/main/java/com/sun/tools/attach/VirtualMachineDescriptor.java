 ...
 * <p/>
 * <p>
 * A <code>VirtualMachineDescriptor</code> is a container class used to describe a Java virtual machine. It encapsulates
 * an identifier that identifies a target virtual machine, and a reference to the
 * {@link com.sun.tools.attach.spi.AttachProvider AttachProvider} that should be used when attempting to attach to the
 * virtual machine. The identifier is implementation-dependent but is typically the process identifier (or pid)
 * environments where each Java virtual machine runs in its own operating system process.
 * </p>
 * <p/>
 * <p>
 * A <code>VirtualMachineDescriptor</code> also has a {@link #displayName() displayName}. The display name is typically
 * a human readable string that a tool might display to a user. For example, a tool that shows a list of Java virtual
 * machines running on a system might use the display name rather than the identifier. A
 * <code>VirtualMachineDescriptor</code> may be created without a <i>display name</i>. In that case the identifier is
 * used as the <i>display name</i>.
 * <p/>
 * <p>
 * <code>VirtualMachineDescriptor</code> instances are typically created by invoking the {@link VirtualMachine#list
 * VirtualMachine.list()} method. This returns the complete list of descriptors to describe the Java virtual machines
 * known to all installed {@link com.sun.tools.attach.spi.AttachProvider attach providers}.
 * 
 * @since 1.6
 */
public final class VirtualMachineDescriptor {
	...
	/**
	 * Returns a hash-code value for this VirtualMachineDescriptor. The hash code is based upon the descriptor's
	 * components, and satisfies the general contract of the Object.hashCode method.
	 * 
	...
	public boolean equals(Object ob) {
		...
		if (!(ob instanceof VirtualMachineDescriptor)) return false;
