...
public class ClassFinder {
	...
	public static Collection<Class<?>> getClassesFromClasspath(final String packageName,
		...
		final String path = packageName.replaceAll("\\.", System.getProperty("file.separator"));
		...
		try {
			...
			while (resources.hasMoreElements()) {
				...
				if (!internalPath.endsWith(".jar")) {
					...
					directories.addAll(getRecursiveDirectories(directory));
				}
			}
			...
			if (directory.exists()) {
				...
				for (final String file : files) {
					...
					if (file.endsWith(".class")) {
						...
						final String absolutePackageName = directory.getAbsolutePath()
						                                            .substring(index)
	...
	public static Collection<Class<?>> getClassesFromJarFile(final String packageName,
		...
		final String path = packageName.replaceAll("\\.", System.getProperty("line.separator"))
			...
			for (final Enumeration<JarEntry> e = currentFile.entries(); e.hasMoreElements();) {
				...
				if ((current.getName().length() > path.length())
					...
					final Class<?> class1 = Class.forName(current.getName()
					                                             .replaceAll(StringEscapeUtils.escapeJava(System.getProperty("line.separator")),
					                                                         ".").replace(".class", ""));
					if (modifiers != null) {
						if ((class1.getModifiers() & modifiers) == 0) {
							classes.add(class1);
	...
	private static Collection<? extends String> getClassNamesFromClassPath(final String cp) throws IOException {
		...
		final Iterator<File> iterator = org.apache.commons.io.FileUtils.iterateFiles(new File(cp), new IOFileFilter() {
		...
		while (iterator.hasNext()) {
			...
			path = path.replaceAll(StringEscapeUtils.escapeJava(System.getProperty("line.separator")), ".");
