...
import net.ownhero.dev.kanuni.annotations.compare.GreaterOrEqualDouble;
...
/**
 * The Class CreatorGreatorOrEqual.
 *
 * @author Sascha Just <sascha.just@own-hero.net>
 */
public final class CreatorGreatorOrEqual extends Creator {
	...
	public String createParameterInstrumentation(final Annotation annotation,
		...
		if (markers.isEmpty()) {
			if (annotation.getTypeName().equals(GreaterOrEqualInt.class.getName())) {
			...
			} else if (annotation.getTypeName().equals(GreaterOrEqualDouble.class.getName())) {
				...
				builder.append(KanuniInstrumenter.compareClass);
				if (parameterType.isPrimitive()) {
					builder.append(String.format(".greaterOrEqual(new Double(%s), new Double(%s), \"%s\", new Object[0]);",
					                             parameterName, ref, text));
				} else {
					builder.append(String.format(".greaterOrEqual(%s, new Double(%s), \"%s\", new Object[0]);",
					                             parameterName, ref, text));
				}
				builder.append(System.getProperty("line.separator"));
			} else {
			...
			for (Integer markerId : markers.keySet()) {
				for (String markerParameter : markers.get(markerId)) {
					builder.append(KanuniInstrumenter.compareClass)
					       .append(String.format(".greaterOrEqual(($w) %s, ($w) %s, \"%s\", new Object[0]);",
