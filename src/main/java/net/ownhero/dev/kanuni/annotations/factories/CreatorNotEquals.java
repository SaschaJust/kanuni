...
import net.ownhero.dev.kanuni.annotations.compare.NotEqualsDouble;
...
/**
 * The Class CreatorNotEquals.
 *
 * @author Sascha Just <sascha.just@own-hero.net>
 */
public final class CreatorNotEquals extends Creator {
	...
	public String createParameterInstrumentation(final Annotation annotation,
		...
		if (markers.isEmpty()) {
			if (annotation.getTypeName().equals(NotEqualsInt.class.getName())) {
			...
			} else if (annotation.getTypeName().equals(NotEqualsDouble.class.getName())) {
				...
				builder.append(KanuniInstrumenter.compareClass);
				if (parameterType.isPrimitive()) {
					builder.append(String.format(".notEquals(new Double(%s), new Double(%s), \"%s\", new Object[0]);",
					                             parameterName, ref, text));
				} else {
					builder.append(String.format(".notEquals(%s, new Double(%s), \"%s\", new Object[0]);",
					                             parameterName, ref, text));
				}
				builder.append(System.getProperty("line.separator"));
			} else {
		...
		for (Integer markerId : markers.keySet()) {
			for (String markerParameter : markers.get(markerId)) {
				builder.append(KanuniInstrumenter.compareClass)
				       .append(String.format(".notEquals(($w) %s, ($w) %s, \"%s\", new Object[0]);", parameterName,
