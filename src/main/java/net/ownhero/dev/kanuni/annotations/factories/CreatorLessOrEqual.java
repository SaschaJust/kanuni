...
/**
 * The Class CreatorLessOrEqual.
 *
 * @author Sascha Just <sascha.just@own-hero.net>
 */
public final class CreatorLessOrEqual extends Creator {
	...
	public String createParameterInstrumentation(final Annotation annotation,
		...
		if (markers.isEmpty()) {
			if (annotation.getTypeName().equals(LessOrEqualInt.class.getName())) {
				...
				if (parameterType.isPrimitive()) {
					builder.append(String.format(".lessOrEqual(new Integer(%s), new Integer(%s), \"%s\", new Object[0]);",
			...
			for (Integer markerId : markers.keySet()) {
				for (String markerParameter : markers.get(markerId)) {
					builder.append(KanuniInstrumenter.compareClass)
					       .append(String.format(".lessOrEqual(($w) %s, ($w) %s, \"%s\", new Object[0]);",
